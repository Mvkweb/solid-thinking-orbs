...
import { ensureGlowPixels, forceGlowReadbackNextFrame } from './sampling';
...
export function updateInstance(
  ...
  if (dirty || patch.kind !== undefined || patch.opacityMul !== undefined || patch.scale !== undefined) {
    inst.everCopied = false;
    inst.glowEverCopied = false;
...
export function setSharedPreset(name: PresetName, theme: PresetTheme): void {
  ...
  s.presetDirty = true;
  forceGlowReadbackNextFrame();
  for (const inst of s.instances) {
    inst.everCopied = false;
    inst.glowEverCopied = false;
  }
  if (s.rafId === 0 && s.pausedAtMs === null && !s.contextLost) {
    startSharedLoop();
  }
}
...
function punchInnerHole(inst: MetalFxInstance): void {
  ...
  if (typeof ctx.roundRect === 'function') {
    ...
    ctx.rect(stroke, stroke, w - 2 * stroke, h - 2 * stroke);
  }
  ctx.fill();
  ctx.restore();
}
...
function copyShaderToInstance(inst: MetalFxInstance): void {
  if (!SHARED) return;
  const src: CanvasImageSource = SHARED.frameBitmap ?? (SHARED.glCanvas as any);
  const dpr = inst.dpr;
  const dw = inst.canvas.width, dh = inst.canvas.height;
  if (dw < 1 || dh < 1) return;

  const cw = (SHARED.glCanvas as any).width, ch = (SHARED.glCanvas as any).height;
...
function uploadPresetUniforms(): void {
  ...
  if (uniforms.u_resolution) gl.uniform2f(uniforms.u_resolution, (glCanvas as any).width, (glCanvas as any).height);
  for (let i = 0; i < 7; i++) {
...
function renderSharedFrame(now: number): void {
  ...
  gl.viewport(0, 0, (glCanvas as any).width, (glCanvas as any).height);
...
function tick(now: number): void {
  ...
  for (const inst of SHARED.instances) {
    if (inst.visible && (!inst.paused || !inst.everCopied || !inst.glowEverCopied)) { anyWork = true; break; }
  }
  ...
  if (_glowCallback && SHARED.glowQueue.length > 0 && ++SHARED.glowSkip % GLOW_SKIP_FRAMES === 0) {
    ...
    if (inst.visible && (!inst.paused || !inst.glowEverCopied)) {
      _glowCallback(inst, now);
      inst.glowEverCopied = true;
    }
    SHARED.glowIdx++;
  }
}
