...
export function AgentThinkingShowcase() {
  ...
  const snippet = createMemo(() => {
    const props = [];
    if (!showTimer()) props.push('showTimer={false}');
    if (!autoCollapse()) props.push('autoCollapse={false}');
    const propStr = props.length ? ` ${props.join(' ')}` : '';
    return `import { AgentThinking } from 'solid-thinking-orbs';\n\n<AgentThinking${propStr} />`;
  });

  return (
    <section class="w-full flex flex-col gap-1.5 mb-12" aria-label="AgentThinking interactive showcase">
      ...
      <div class="flex flex-col gap-4 bg-(--panel-bg) rounded-[10px] p-4 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)]">
        <div class="flex items-center justify-between gap-4 max-sm:flex-col max-sm:items-start">
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <TabBtn active={showTimer()} onClick={() => setShowTimer((t) => !t)}>
              Timer: {showTimer() ? 'On' : 'Off'}
            </TabBtn>

            <TabBtn active={autoCollapse()} onClick={() => setAutoCollapse((c) => !c)}>
              Auto-Collapse: {autoCollapse() ? 'On' : 'Off'}
            </TabBtn>
          </div>
        </div>
      </div>
